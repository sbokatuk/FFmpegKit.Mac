...
# Builds the host test app against a packed FFmpegKit package and runs its smoke tests directly
# on this Mac - macOS is the target platform, so no simulator or device is involved. The app
# prints its verdict to stdout; this script turns that into an exit code.
#
# Usage: run-host-tests.sh [VARIANT] VERSION [TARGET_FRAMEWORK]

VARIANT="${1:-Video}"
VERSION="${2:?a package version is required}"
TARGET_FRAMEWORK="${3:-net10.0-macos26.0}"

LOG_FILE="host-tests.log"
# CI runners are Apple silicon. Override for an Intel machine.
HOST_RID="${FFMPEGKIT_HOST_RID:-osx-arm64}"

REPO_ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
PROJECT="${REPO_ROOT}/tests/FFmpegKit.Mac.HostTests/FFmpegKit.Mac.HostTests.csproj"

# The .NET 9 band builds net8/net9 and the .NET 10 band builds net9/net10, so pick the SDK that
# owns the requested target framework. As with iOS (and unlike Android), there is no net8 ->
# .NET 8 band mapping: the .NET 9 band's macOS workload builds net8.0-macos outright, runtime
# packs included. The SDK is resolved from the working directory, and the repository's
...
package_id="ffmpegkit.net.$(printf '%s' "${VARIANT}" | tr '[:upper:]' '[:lower:]').mac"
...
APP_PATH="$(find "${REPO_ROOT}/tests/FFmpegKit.Mac.HostTests/bin/Release/${TARGET_FRAMEWORK}/${HOST_RID}" \
...
if ! grep -q "FFMPEGKIT_E2E_DONE PASS" "${LOG_FILE}"; then
    echo "::error::FFmpegKit host smoke tests failed or timed out"
    exit 1
fi

echo "==> host smoke tests passed"
