...
PROJECT="${REPO_ROOT}/tests/FFmpegKit.Mac.DeviceTests/FFmpegKit.Mac.DeviceTests.csproj"
...
APP_PATH="$(find "${REPO_ROOT}/tests/FFmpegKit.Mac.DeviceTests/bin/Release/${TARGET_FRAMEWORK}/${SIMULATOR_RID}" \
...
if ! grep -q "FFMPEGKIT_E2E_DONE PASS" "${LOG_FILE}"; then
    ...
    xcrun simctl spawn "${udid}" log show --last 2m --predicate "process == 'FFmpegKit.Mac.DeviceTests'" \
