...
# src/FFmpegKit.Mac/libs/<Variant>/.
#
# NOTE: unlike Android, there is no Maven-style feed for the Apple binaries. The original
...
# eight variants with a macOS slice. See the README for the full comparison.
#
# Usage:
...
LIBS="$ROOT/src/FFmpegKit.Mac/libs"
...
# build with a confusing "file not found" on frameworks nobody downloaded.
...
# Each xcframework ships an ios device slice, an ios simulator slice and a macos one. This
# repository binds macOS, so the two ios slices - roughly two thirds of the payload - are dead
# weight that would still be embedded in the package: the whole xcframework is zipped into the
# assembly's .resources.zip, once per target framework. Stripping them keeps every variant
# comfortably under nuget.org's 250 MB limit. (The iOS repository does the mirror image.)
#
# The slice directory alone cannot just be deleted - AvailableLibraries in Info.plist would still
# advertise it, and the SDK rejects an xcframework whose manifest points at a missing slice -
# so the manifest is rewritten to match. plistlib is stdlib, and python3 is already required by
# the package merge step.
strip_non_macos_slices() {
    python3 - "$1" <<'PYTHON'
import plistlib
import shutil
import sys
...
for manifest in sorted(root.glob("*.xcframework/Info.plist")):
    ...
    keep = [lib for lib in libraries if lib.get("SupportedPlatform") == "macos"]
    dropped = [lib for lib in libraries if lib.get("SupportedPlatform") != "macos"]

    if not keep:
        raise SystemExit(f"error: {manifest.parent.name} has no macos slice")
    if not dropped:
        continue
